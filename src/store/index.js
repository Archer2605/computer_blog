...
const store = createStore({
  ...
  actions: {
    ...
    async getListBlogBySearch({commit},searchKey){
      try{
        const response = await axios.get('http://localhost:3000/api/blog?searchKey='+searchKey)
        commit('GETLISTBLOG',response.data)
      }catch(error){
          console.log("Loi action getListBlogBySearch")
      }
    },
  }
})
